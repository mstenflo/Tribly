...
export const getContributions = (topicId) => async (dispatch) => {
  try {
    ...
    const res = contributions.data.filter(
      (contribution) => contribution.topic.id === topicId
    );
    dispatch({
...
export const addContribution =
  (groupId, topicId, formData) => async (dispatch) => {
    try {
      const config = {
        headers: {
      ...
      const res = await axios.post(
        `/api/contributions/${groupId}/topic/${topicId}`,
        formData,
        config
      );
